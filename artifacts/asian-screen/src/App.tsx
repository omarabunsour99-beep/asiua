...
import { getPublicTitle, toLegacyTitle } from '@/lib/api';
...
function Detail({ type }: { type: TitleType }) {
  ...
  useEffect(() => { if (id) void getPublicTitle(id).then(setRemoteDetail).catch(() => undefined); }, [id]);
  const item = remoteDetail ? toLegacyTitle(remoteDetail) : getTitle(id);
  const [toast, setToast] = useState('');
...
function Watch({ item }: { item: Title | undefined }) {
  ...
  const current = (remoteDetail ? toLegacyTitle(remoteDetail) : null) || item || getTitle(id);
