...
function CountryPage({ country }: { country: Country }) {
  ...
  const remote = usePublicTitles({ country, page: 1, pageSize: 100 });
  const list = remote.items.map(toLegacyTitle).filter(t => !genre || t.genres.includes(genre));
